...
std::vector<Detection> Inference_trt::runInferenceTensorRT(const cv::Mat &input)
{
    ...
    memcpy(inputData.data(), inputBlob.data, inputSize);

    cudaMemcpyAsync(deviceBuffers[inputIndex], inputData.data(), inputSize, cudaMemcpyHostToDevice, cudaStream);
    ...
    cudaMemcpyAsync(outputData.data(), deviceBuffers[outputIndex], outputSize, cudaMemcpyDeviceToHost, cudaStream);
